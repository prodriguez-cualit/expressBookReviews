...
module.exports = {
      books,
      getAllbooks: async () => {
            return new Promise( resolve => setTimeout(() => resolve(books), 700));
      },
      getBookByISBN: async (isbn) => {
            return new Promise( resolve => setTimeout(() => resolve(books[isbn]), 200));
      },
      getBooksByTitle: async (title) => {
            const filteredBooks = Object.values(books).filter(book => book.title.toLowerCase() === title.toLowerCase());
            return new Promise( resolve => setTimeout(() => resolve(filteredBooks), 300));
      },
      getBookByAuthor: async (author) => {
            ...
            return new Promise( resolve => setTimeout(() => resolve(filteredBooks), 400));
      }
};
